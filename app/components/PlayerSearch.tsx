...
export default function PlayerSearch({ value, onChange, placeholder = "Search for player...", className = "" }: PlayerSearchProps) {
  ...
  const searchCards = async (query: string) => {
    ...
    setIsLoading(true);
    try {
      // Use local NFL player data instead of API
      const { searchNFLPlayers } = await import('@/app/data/nfl-players');
      const players = searchNFLPlayers(query, 10);
      
      // Convert players to Card format
      const playerCards: Card[] = players.map((player: any) => ({
        id: player.id,
        name: player.name,
        ...
        notes: `${player.team || ''} ${player.position ? `• ${player.position}` : ''}`.trim(),
      }));
      
      setCards(playerCards);
      setShowDropdown(playerCards.length > 0);
      setSelectedIndex(-1);
