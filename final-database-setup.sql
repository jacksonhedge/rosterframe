-- FINAL DATABASE SETUP FOR ROSTER FRAME CARD SYSTEM
-- This script will completely set up your database with a safer approach
...
-- Insert data without ON CONFLICT clauses
...
-- Get NFL sport ID and insert teams and positions
DO $$
DECLARE 
    nfl_sport_id UUID;
BEGIN
...
-- Set up storage policies (drop first to avoid conflicts)
DO $$
BEGIN
...
EXCEPTION
    WHEN OTHERS THEN
        -- Policy creation failed, but that's okay
        NULL;
END $$;
...
-- Success message
SELECT 'Database setup complete! NFL sport ID: ' || id::text as message 
